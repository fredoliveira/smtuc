class SMTUC::Line
  ...
  # Directions are returned in a funky format that makes little sense, but looks like:
  # R§Volta§Tovim - Est. Nova
  #
  # So we define a regular expression that captures all 3 parts into capture groups:
  # 1: R
  # 2: Volta
  # 3: Tovim - Est. Nova
  ...
  def self.find(id)
    ...
    line = self.all.select{ |line| line.id == id }.first
  ...
  def self.parse_directions(directions)
    ...
    # }
    return { direction: matches[2].downcase, description: matches[3] }
  end
end
